...
{
	auto components = ComponentSystem::get();
	if (components->getComponent<ObjSwitch>(node) != nullptr)
		return true;

	ObjScript *script = components->getComponent<ObjScript>(node);
	if (script == nullptr)
		return false;

	return script->hasSwitcherBehaviour();
}

int VRPlayer::is_it_handle(const NodePtr &node)
{
	auto components = ComponentSystem::get();
	if (components->getComponent<ObjHandle>(node) != nullptr)
		return true;
	...
	return script->hasHandleBehaviour();
}
