...
void ObjMovable::init()
{
	...
	if (use_physics.get())
	{
		...
		if (!body)
		{
			...
			if (obj)
			{
				body = BodyRigid::create(Object::cast(node));
	...
	if (body)
	{
		...
		contact_callback_id = body->addContactCallback(MakeCallback(this, &ObjMovable::contact_callback));
...
void ObjMovable::shutdown()
{
	if (body)
	{
		...
		body->removeContactCallback(contact_callback_id);
...
void ObjMovable::unfreeze(const BodyPtr &body)
{
	...
	if (getComponent<ObjMovable>(body->getObject()->getNode()))
		activate_body(body);
}
...
void ObjMovable::play_sound(const BodyPtr &body, const Vec3 &pos, float volume)
{
	...
	ObjMovable* obj = getComponent<ObjMovable>(body->getObject()->getNode());
	...
	SoundManager::get()->playSound(
		...
		pos, 
		volume,
		Game::get()->getRandomFloat(0.8f, 1.2f), // pitch
		obj->impact_min_dist,
		obj->impact_max_dist);
}
...
void ObjMovable::play_sound_action(const BodyPtr &body, int num)
{
	if (num >= body->getNumContacts()) return;

	vec3 velocity = vec3(last_pos[last_pos_index] - last_pos[1 - last_pos_index]) / Physics::get()->getIFps();
