...
public:
	...
	// events
	Action<void(Unigine::BodyPtr, int)> onContact;
...
private:
	...
	UNIGINE_INLINE void contact_callback(Unigine::BodyPtr body, int num) { onContact(body, num); }
	...
	void unfreeze_action(const Unigine::BodyPtr &body, int num);
	void play_sound_action(const Unigine::BodyPtr &body, int num);
